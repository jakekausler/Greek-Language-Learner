...
def generateNewFB(sent,node,fs):
    ...
    if isinstance(node,Terminal):
        print 'Added to sentence node ' + node.toString()
        sent.append(node.name)
        return (sent,True)
    #If not, do recursion
    else:
...
#                 print child.toString()
            if node.childFeatures[i][0].unify(fs) != None:
                ...
                while unifies and j < len(node.childFeatures[i]):
                    print 'testing to see if',node.childFeatures[i][0],'unifies with',node.childFeatures[i][j]
                    if node.childFeatures[i][0].unify(node.childFeatures[i][j]) == None:
                        print node.childFeatures[i][0], 'did not unify with',node.childFeatures[i][j]
                        unifies = False
                    j+=1
                if unifies:
                    print 'Appended to path node',node.children[i]
                    paths.append(i)
        ...
        #Otherwise, choose a random path until one works or all are exausted
            ...
            while len(paths) > 0:
                print 'there are still paths to test'
                newSent = sent
                i = 0
                works = True
                ...
                while i < len(node.children[path]) and works:
                    ...
                    if fields[1]:
                        ...
                        print 'path did not work because the second field was false'
                        works = False
                    i+=1
                if works:
                    print 'the field worked'
                    return (newSent,True)
                else:
                    paths.remove(path)
                    print 'removed the path'
                    if len(paths) == 0:
                        print 'no paths worked'
                        return (sent,False)
                    else:
                        path = paths[random.randint(0,len(paths)-1)]

def generateSemanticFB(sent,node,fs,maxDepth=3,currentDepth=0,previousNode=None):
    # If the node is a terminal, add it to the sentence
    if isinstance(node,Terminal):
        sent.append(node.name)
        print sent
        return sent
    # If the node is not terminal, go to the next child
    else:
        # Temporarily-- get a random child path and go to the end
        paths = []
        for i in range(len(node.children)):
            if currentDepth == maxDepth:
                if not any(child.name==previousNode.name for child in node.children[i]):
                    paths.append(node.children[i])
            else:
                paths.append(node.children[i])
        for child in random.choice(paths):
            if previousNode != None and previousNode.name==child.name:
                currentDepth+=1
            else:
                currentDepth=0
            sent = generateSemanticFB(sent,child,fs,maxDepth,currentDepth,child)
        return sent
                    
def generateSentence(nodes,head,maxDepth=3):
    ...
    sent = []
    possibleHeads = getHeads(nodes,head)
    chosenHead = random.choice(possibleHeads)
#     print nodes[chosenHead].toString()
    for item in generateSemanticFB([],nodes[chosenHead],nodes[chosenHead].childFeatures[0][0])[0]:
        sent.append(item)
#     print sent
    return sent
...
def getHeads(nodes,headName):
    headIds = []
    for i in range(len(nodes)-1):
        if nodes[i].name == headName:
            headIds.append(i)
    return headIds
...
for i in range(1,10):
    print generateSentence(readCFG('../Semantics/grammar.fcfg'),'S')
#     print generateSentence(readCFG('Grammar.fcfg'))
# f = open('test','w')
# f.write(printCFG(readCFG('../Semantics/grammar.fcfg')))
# f.close()
